...
function ConstellationsPanel({
  ...
  // Static stars generated once at module level to avoid re-renders
...
function SnowfallGame() {
  ...
  // Hydrate save from localStorage
  useEffect(() => {
    if (save) return;
  ...
  useEffect(() => {
    if (!save) return;
    const ts = Date.now();
    if (ts - saveThrottleRef.current < 5000) return;
    saveThrottleRef.current = ts;
    saveSave(save);
  }, [save]);
    ...
    function frame(ts: number) {
      ...
      setSave((s) => {
        ...
        for (const def of BUILDINGS) {
          ...
          base += owned * def.basePps * (b.buildingMult[def.key] ?? 1) * b.globalMult;
        }
  ...
  const buffsRef = useRef<BuffSet>(emptyBuffs());
  const basePpsRef = useRef(0);
  useEffect(() => { buffsRef.current = buffs; }, [buffs]);
  useEffect(() => { basePpsRef.current = basePps; }, [basePps]);

  useEffect(() => {
    const id = setInterval(() => {
      setSave((s) => {
        if (!s) return s;
        if (Date.now() >= s.shimmerNextAt) {
          setShimmer((prev) => {
            if (prev) return prev;
            shimmerIdRef.current += 1;
            return { id: shimmerIdRef.current, x: 10 + Math.random() * 80, y: 20 + Math.random() * 60, created: Date.now() };
          });
          const freq = 240_000 + Math.random() * 360_000;
          return { ...s, shimmerNextAt: Date.now() + freq * buffsRef.current.shimmerFreqMult };
        }
        return s;
      });
      setShimmer((prev) => {
        if (prev && Date.now() - prev.created > 14_000) return null;
        return prev;
      });
    }, 500);
    return () => clearInterval(id);
  ...
  const catchShimmer = () => {
    ...
    const currentBasePps = basePpsRef.current;
    setSave((s) => {
      if (!s) return s;
      const roll = Math.random();
      if (roll < 0.45) {
        const mult = 7 * currentBuffs.shimmerPowerMult;
        toast.success(`Shimmer — ${mult.toFixed(0)}× production for 60 seconds`);
        return { ...s, activeBuff: { until: Date.now() + 60_000, mult } };
      }
      if (roll < 0.85) {
        const gain = currentBasePps * 900 * currentBuffs.shimmerPowerMult;
      ...
      const mult = 15 * currentBuffs.shimmerPowerMult;
  ...
  const clickFlake = useCallback(() => {
    setSave((s) => {
      ...
      const currentBuffMult = s.activeBuff && s.activeBuff.until > Date.now() ? s.activeBuff.mult : 1;
      const click = (1 + basePpsRef.current * 0.01) * b.clickMult * currentBuffMult;
        ...
        <div className="mx-auto max-w-7xl px-3 py-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex gap-1.5 flex-wrap">
              ...
              <Button variant="ghost" size="sm" onClick={() => setConfirmReset(true)}><RotateCcw className="h-4 w-4" /></Button>
            </div>
          </div>
